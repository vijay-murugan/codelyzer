    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "\n"
                "PRIMARY GOAL:\n"
                "Generate REAL runnable tests that validate behavior of changed code.\n"
                "Prefer best-effort tests with mocks over skipped scaffolds.\n"
                "Skip tests only as an absolute last resort.\n"
                "\n"
                "OUTPUT RULES:\n"
                "- Return only valid Python test code.\n"
                "- No markdown fences.\n"
                "- No prose or narration.\n"
                "- No TODOs, placeholders, or NotImplementedError.\n"
                "- No large comments.\n"
                "- Every test_* must contain at least one assert, pytest.raises(...), or mock assert_called*.\n"
                "- Never output empty tests.\n"
                "\n"
                "WHEN CONTEXT IS PARTIAL:\n"
                "- Infer behavior from function name, signature, docstring, diff, and source code.\n"
                "- Mock unknown dependencies.\n"
                "- Test observable effects: return value, raised errors, calls, state changes.\n"
                "- If exact output unknown, assert type / non-None / dependency interaction.\n"
                "- Use pytest.mark.skip ONLY if target function body is completely unavailable.\n"
                "\n"
                "TEST PHILOSOPHY:\n"
                "- Prefer unit tests over integration tests.\n"
                "- Test public behavior, not internal implementation trivia.\n"
                "- Cover changed logic first.\n"
                "- Cover regressions implied by the diff.\n"
                "- Keep tests deterministic and isolated.\n"
                "\n"
                "MANDATORY COVERAGE:\n"
                "1. Happy path.\n"
                "2. At least one edge case.\n"
                "3. At least one failure path if possible.\n"
                "4. New or changed branches from diff.\n"
                "5. Changed return values / exceptions / side effects.\n"
                "\n"
                "EDGE CASES:\n"
                "- None\n"
                "- Empty values\n"
                ...
                "\n"
                "EXTERNAL I/O:\n"
                "- Never call real HTTP, DB, cloud SDKs, subprocess, sockets, queues, filesystem writes.\n"
                "- Mock everything external.\n"
                "- Use patch(), MagicMock, Mock, monkeypatch.\n"
                "\n"
                "PATCH WHERE USED:\n"
                "- If module imports requests, patch module.requests.get.\n"
                "- If module imports from requests import get, patch module.get.\n"
                "- Patch symbols exactly where resolved.\n"
                "\n"
                "UNKNOWN SDK / HARDWARE / ROBOTICS RULE:\n"
                "- If code uses proprietary SDKs (Naoqi, boto3, cv2, serial, etc), always mock them.\n"
                "- Assert constructor calls, method calls, parameters, returned values.\n"
                "\n"
                "IMPORT PATH RULES:\n"
                "- Derive module path from repo_path + file_path.\n"
                "- Remove repo root.\n"
                "- Replace / with .\n"
                "- Remove .py suffix.\n"
                "- Example: /repo/pkg/util.py -> from pkg.util import ...\n"
                "\n"
                "STYLE:\n"
                "- pytest only.\n"
                "- Use parametrize where useful.\n"
                "- Deterministic names: test_<unit>_<behavior>.\n"
                "- Import only what is needed.\n"
                "- Keep concise.\n"
                "\n"
                "GOOD EXAMPLE:\n"
                "from unittest.mock import patch, MagicMock\n"
                "from app.robot import runNaoqi\n"
                "\n"
                "def test_runNaoqi_calls_session():\n"
                "    fake = MagicMock()\n"
                "    with patch('app.robot.ALProxy', return_value=fake) as proxy:\n"
                "        runNaoqi()\n"
                "    proxy.assert_called()\n"
                "\n"
                ...
                "\n"
                "BAD OUTPUT:\n"
                "- skipped tests when source exists\n"
                "- NotImplementedError\n"
                "- real network/db calls\n"
                "- missing asserts\n"
                "- redefining production code\n"
            ),
            (
                "human",
                ...
                "Research guidance:\n{research_guidance}\n\n"
                "Generate pytest tests for changed code.\n"
