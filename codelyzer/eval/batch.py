...
def _pretty_json_path(jsonl_path: Path) -> Path:
    if jsonl_path.suffix == ".jsonl":
        return jsonl_path.with_name(f"{jsonl_path.stem}.pretty.json")
    return jsonl_path.with_name(f"{jsonl_path.name}.pretty.json")


def _raw_jsonl_path(out_path: Path) -> Path:
    if out_path.suffix == ".jsonl":
        return out_path.with_name(f"{out_path.stem}.raw.jsonl")
...
def _coverage_scope_from_case(case: dict[str, Any]) -> str:
    ...
    if isinstance(raw, str) and raw.strip() == "full_source":
        return "full_source"
    if isinstance(raw, str) and raw.strip() == "diff_files":
        return "diff_files"
    return "runtime"
...
def eval_batch(config_path: Path, output: Path | None, verbose: bool) -> None:
    ...
    coverage_scope (runtime|full_source|diff_files),
    label.
    """
    ...
    pretty_path = _pretty_json_path(out_path)
    raw_jsonl_path = _raw_jsonl_path(out_path)
    ...
    pretty_json = json.dumps(rows, indent=2, default=str) + "\n"
    out_path.write_text(pretty_json, encoding="utf-8")
    pretty_path.write_text(pretty_json, encoding="utf-8")
    with raw_jsonl_path.open("w", encoding="utf-8") as fh:
        for row in rows:
            fh.write(json.dumps(row, default=str) + "\n")
    click.echo(f"Wrote {len(cases)} row(s) to {out_path}")
    click.echo(f"Wrote pretty JSON to {pretty_path}")
    click.echo(f"Wrote raw JSONL to {raw_jsonl_path}")
